...
chmod -R u+w "$TEST_HOST_APP"
# Re-sign the test host app with ad-hoc identity (inside-out) to strip
# restrictive entitlements (e.g. App Sandbox without get-task-allow)
# that would prevent the app from being launched by the test runner.
if [[ -d "$TEST_HOST_APP/Contents/Frameworks" ]]; then
  while IFS= read -r fw; do
    codesign_item "$fw"
  done < <(find "$TEST_HOST_APP/Contents/Frameworks" -type d -name "*.framework" -prune | sort)
  while IFS= read -r dl; do
    codesign_item "$dl"
  done < <(find "$TEST_HOST_APP/Contents/Frameworks" -type f -name "*.dylib" | sort)
fi
codesign --force --timestamp=none --sign - "$TEST_HOST_APP" >/dev/null 2>&1
...
chmod -R u+w "$RUNNER_APP_DESTINATION"
...
chmod -R u+w "$RUNNER_PLUGINS_DIR/$TEST_BUNDLE_NAME.xctest"
...
# When BUILD_WORKSPACE_DIRECTORY is set (e.g. marketing screenshot targets that
# need workspace-relative fixture files), expose it as CLIPKITTY_PROJECT_ROOT
# so the test's resolveProjectRoot() can find data files.
