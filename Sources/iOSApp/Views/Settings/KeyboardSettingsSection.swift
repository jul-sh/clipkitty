    ...
    var body: some View {
        Section(String(localized: "Keyboard")) {
            if keyboardLastOpened != nil {
                ...
                Text(String(localized: "Tap the globe key on any keyboard to switch to ClipKitty and insert recent clips."))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            } else {
                VStack(alignment: .leading, spacing: 8) {
                    Text(String(localized: "Paste from your clip history in any app — the ClipKitty keyboard shows your recent clips as cards. Tap one to insert it, or drag it into the app."))
                        .foregroundStyle(.primary)

                    Text(String(localized: "Open ClipKitty in the Settings app, tap \"Keyboards\", then turn on ClipKitty and \"Allow Full Access\". Full Access is what lets the keyboard read your clips; nothing leaves your device."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
