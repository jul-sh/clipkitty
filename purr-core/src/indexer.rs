...
use crate::search_admission::{
    ...
    WORD_MATCH_SIGNAL, CHUNK_PARENT_THRESHOLD_BYTES as SEARCH_ADMISSION_CHUNK_PARENT_THRESHOLD_BYTES,
};
use chrono::Utc;
...
pub const CHUNK_PARENT_THRESHOLD_BYTES: usize = SEARCH_ADMISSION_CHUNK_PARENT_THRESHOLD_BYTES;
...
pub mod test_support {
    ...
    pub struct SearchTestHooks {
        pub before_phase_two: Option<Arc<dyn Fn() + Send + Sync>>,
        pub on_phase_two_candidate: Option<Arc<dyn Fn(usize) + Send + Sync>>,
    }

    static HOOKS: Lazy<Mutex<SearchTestHooks>> =
        Lazy::new(|| Mutex::new(SearchTestHooks::default()));
    pub static HOOK_TEST_LOCK: Lazy<Mutex<()>> = Lazy::new(|| Mutex::new(()));

    pub struct SearchTestHookGuard;
    ...
    pub fn install_search_hooks(hooks: SearchTestHooks) -> SearchTestHookGuard {
        *HOOKS.lock() = hooks;
        SearchTestHookGuard
    }

    pub fn before_phase_two() {
    ...
    pub fn on_phase_two_candidate(index: usize) {
...
impl Indexer {
    pub fn inspect(path: &Path) -> IndexerResult<IndexInspection> {
        if !path.exists() {
