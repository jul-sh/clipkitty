...
pub fn add_tag(db: &Database, item_id: i64, tag: ItemTag) -> Result<(), ClipKittyError> {
    db.add_tag(item_id, tag)?;
    Ok(())
}

pub fn remove_tag(db: &Database, item_id: i64, tag: ItemTag) -> Result<(), ClipKittyError> {
    db.remove_tag(item_id, tag)?;
    Ok(())
}

pub fn delete_item(db: &Database, indexer: &Indexer, item_id: i64) -> Result<(), ClipKittyError> {
...
pub fn clear(db: &Database, indexer: &Indexer) -> Result<(), ClipKittyError> {
    db.clear_all()?;
    indexer.clear()?;
    Ok(())
}
