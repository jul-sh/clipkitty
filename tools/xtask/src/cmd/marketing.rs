...
impl ScreenshotPlan {
    ...
    fn ios() -> Self {
        Self {
            ...
            db_mode: ScreenshotDbMode::SharedEnglishDatabase,
            missing_policy: MissingScreenshotPolicy::Warn,
...
fn postprocess_video(
    ...
    Runner::new(reporter, "ffmpeg")
        ...
        .arg("-vf")
        .arg(format!(
            "{crop_filter}scale=2880:1800:force_original_aspect_ratio=decrease,pad=2880:1800:(ow-iw)/2:(oh-ih)/2:color=0xC0C0C0"
        ))
        .args([
            "-c:v",
            "libx264",
