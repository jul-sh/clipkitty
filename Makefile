...
# Pass LOCKED=1 in CI to enforce Cargo.lock and MODULE.bazel.lock.
CARGO_LOCKED := $(if $(filter 1,$(LOCKED)),--locked,)
BAZEL_LOCKED := $(if $(filter 1,$(LOCKED)),--config=lockfile,)
export LOCKED
...
BAZEL_UNIVERSAL := $(if $(filter 1,$(UNIVERSAL)),--config=universal,)
BAZEL_BUILD_FLAGS := --embed_label="$(EMBED_LABEL)" -c $(BAZEL_COMPILATION_MODE) $(BAZEL_LOCKED) $(BAZEL_UNIVERSAL) $(BAZEL_ARGS)
...
generate:
	@echo "Error: Bazel is the authoritative Apple build graph; no project generation step is required." >&2
	@exit 1
...
build:
	...
	@MATCHES="$$( $(BAZEL) cquery $(BAZEL_BUILD_FLAGS) --output=files $(BAZEL_TARGET) | grep -E '\.(zip|ipa)$$' )"; \
	COUNT=$$(echo "$$MATCHES" | grep -c .); \
	if [ "$$COUNT" -ne 1 ]; then \
		echo "Error: expected exactly 1 artifact but found $$COUNT:" >&2; \
		echo "$$MATCHES" >&2; \
		exit 1; \
	fi; \
	ARTIFACT="$$MATCHES"; \
...
uitest:
	...
	@$(BAZEL) test $(BAZEL_BUILD_FLAGS) --test_timeout=600 //:ClipKittyUITests $(if $(TEST),--test_filter=$(TEST),)

mac-appstore-uitest:
	@$(MAKE) rust
	@echo "Running App Store macOS UI tests with Bazel..."
	@$(BAZEL) test $(BAZEL_BUILD_FLAGS) --test_timeout=600 //:ClipKittyAppStoreUITests $(if $(TEST),--test_filter=$(TEST),)
...
ios-uitest:
	...
	@$(BAZEL) test $(BAZEL_BUILD_FLAGS) --test_timeout=600 --config=ios_sim //:ClipKittyiOSUITests $(if $(TEST),--test_filter=$(TEST),)

ios-appstore-uitest:
	@$(MAKE) rust
	@echo "Running App Store iOS UI tests with Bazel..."
	@$(BAZEL) test $(BAZEL_BUILD_FLAGS) --test_timeout=600 --config=ios_sim //:ClipKittyiOSAppStoreUITests $(if $(TEST),--test_filter=$(TEST),)
...
update-sim-config:
	@DEVICE=$$($(SCRIPT_DIR)/Scripts/detect-ios-simulator.sh device) && \
	OS=$$($(SCRIPT_DIR)/Scripts/detect-ios-simulator.sh os) && \
	echo "Detected: device=$$DEVICE os=$$OS" && \
	sed -i '' "s/^IOS_SIM_DEVICE = .*/IOS_SIM_DEVICE = \"$$DEVICE\"/" $(SCRIPT_DIR)/bazel/runners/BUILD.bazel && \
	sed -i '' "s/^IOS_SIM_OS = .*/IOS_SIM_OS = \"$$OS\"/" $(SCRIPT_DIR)/bazel/runners/BUILD.bazel && \
	echo "Updated bazel/runners/BUILD.bazel"
