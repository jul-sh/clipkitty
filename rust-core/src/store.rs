...
impl ClipboardStore {
    ...
    /// Set item timestamp to a specific Unix timestamp (for testing/screenshot mode)
    pub fn set_timestamp(&self, item_id: i64, timestamp_unix: i64) -> Result<(), ClipKittyError> {
        let timestamp = Utc.timestamp_opt(timestamp_unix, 0)
            .single()
            .ok_or_else(|| ClipKittyError::InvalidInput("Invalid timestamp".into()))?;
