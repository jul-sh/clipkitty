...
final class ClipKittyUITests: XCTestCase {
    ...
    /// Read from /tmp/clipkitty_screenshot_locale.txt (written by distribution/marketing-screenshots-localized.sh before test run).
    ...
    private func setupTestDatabase(in appSupportDir: URL) throws {
        ...
        // Read the database filename from temp file (written by distribution/marketing-screenshots-localized.sh)
    ...
    /// Run with: distribution/intro-video-localized.sh (or via the intro-videos CI job).
    ///
    /// Uses SyntheticData_video.sqlite (generated by distribution/intro-video-localized.sh)
    ...
    /// Scene 4: Image search — type locale-aware "fast" to find cartoon cat
